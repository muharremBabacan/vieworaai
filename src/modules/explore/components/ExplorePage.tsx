...
export default function ExplorePage() {
  const [view, setView] = useState<'halls' | 'gallery'>('halls');
      ...
      <div className="container mx-auto px-4 pb-20 animate-in fade-in duration-500">
        ...
        {isHallsLoading ? (
          ...
          <div className="text-center py-32 rounded-[40px] border-2 border-dashed border-border/40 bg-muted/5">
            ...
            <p className="text-muted-foreground max-w-sm mx-auto mt-2">Yeni sergiler yakında burada olacak. Takipte kalın!</p>
          </div>
        )}
      </div>
    );
    ...
    <div className="container mx-auto px-4 pb-20 animate-in slide-in-from-right-4 duration-500">
      ...
      {isPhotosLoading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {photos.map((photo) => (
            <Card key={photo.id} className="group relative aspect-square rounded-[24px] overflow-hidden border-none shadow-lg cursor-pointer">
