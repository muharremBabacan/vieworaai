    ...
    <div className="container mx-auto px-4 pb-20">
      ...
      <Tabs defaultValue="accounting" className="space-y-8">
        ...
        <TabsContent value="accounting" className="space-y-8">
          ...
          <Card className="rounded-[24px] border-border/40">
                  ...
                  {logs?.map(log => (
                    <div key={log.id} className="p-4 flex items-center justify-between hover:bg-muted/30 transition-colors">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[10px] text-muted-foreground">{formatDistanceToNow(new Date(log.timestamp), { addSuffix: true, locale: tr })}</p>
                      </div>
                    </div>
                  ))}
                </div>
