...
import { Star, Loader2, Eye, Clock, Zap, Trash2, Camera, Smartphone, HelpCircle, Bot } from 'lucide-react';
                ...
                <div className="space-y-3">
                  {photo.isSubmittedToExhibition ? (
                      <Button variant="outline" className="w-full" onClick={() => setDialogAction('withdrawExhibition')} disabled={isLoading}>
                          {isLoading && dialogAction === 'withdrawExhibition' ? <Loader2 className="animate-spin"/> : <><Eye className="mr-2 h-4 w-4"/>{t('button_withdraw_from_public')}</>}
                      </Button>
                  ) : (
                      <Button className="w-full" onClick={handleSubmitToExhibition} disabled={isLoading}>
                          {isSubmitting ? <Loader2 className="animate-spin"/> : <><Eye className="mr-2 h-4 w-4"/>{t('button_submit_to_public', { cost: 5 })}</>}
                      </Button>
                  )}
                </div>
             )}
